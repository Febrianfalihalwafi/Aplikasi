...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        // Bust cache avatar lama supaya tidak nyampur antar user
        final oldAvatar = _userData?['avatar'];
        if (oldAvatar != null) {
          imageCache.evict(NetworkImage(oldAvatar));
        }
        setState(() {
          _userData = data['user'];
          _pickedImage = null;
          _avatarCacheBuster = null; // reset cache buster saat load fresh
  ...
  Future<void> _saveProfile() async {
    ...
    // Upload avatar dulu kalau ada foto baru yang dipilih
    if (_pickedImage != null) {
      await _uploadAvatar(_pickedImage!);
      if (!mounted) return;
      // Kalau _pickedImage masih tidak null, berarti upload gagal — stop
      if (_pickedImage != null) {
        _showSnack('Gagal upload foto, coba lagi', isError: true);
        return;
      }
    }
  ...
  Future<void> _uploadAvatar(File imageFile) async {
    ...
    try {
      ...
      print('[AVATAR] status=${response.statusCode} data=$data');
      ...
      if (response.statusCode == 200) {
        ...
        if (!mounted) return;
        // Bust image cache supaya tidak nyampur antar user
        imageCache.evict(NetworkImage(avatarUrl));
        if (_userData?['avatar'] != null) {
          imageCache.evict(NetworkImage(_userData!['avatar']));
        }
        setState(() {
        ...
        _showSnack(data['msg'] ?? 'Upload avatar gagal', isError: true);
      }
    } catch (e) {
      if (!mounted) return;
      setState(() => _isUploadingAvatar = false);
      _showSnack('Upload avatar gagal: $e', isError: true);
    }
  }
